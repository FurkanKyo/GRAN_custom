...
class GranRunner(object):
  ...
  def __init__(self, config):
    ...
    logger.info('Save dir = {}'.format(self.config.save_dir))

    ### load graphs
    self.graphs = create_graphs(config.dataset.name, data_dir=config.dataset.graphs_path)
  ...
  def train(self):
    ...
    # Training Loop
    iter_count = 0 if not self.train_conf.is_resume else \
      (self.train_conf.resume_epoch * (len(self.graphs_train) // 16)) + 1 # Division is by previous batch_size
    logger.info("Starting with iter_count = {}".format(iter_count))
    ...
    for epoch in range(resume_epoch, self.train_conf.max_epoch):
      ...
      if (epoch + 1) % self.train_conf.snapshot_epoch == 0:
        ...
        _, _, _, _, mmd_degree_test, mmd_clustering_test, _, _ = self.test(model=model, epoch_count=epoch + 1)
        self.writer.add_scalar('mmd_degree_test', mmd_degree_test, iter_count)
        self.writer.add_scalar('mmd_clustering_test', mmd_clustering_test, iter_count)


    pickle.dump(results, open(os.path.join(self.config.save_dir, 'train_stats.p'), 'wb'))
    return 1

  def test(self, model=None, epoch_count=None):
    ...
    s_graphs_dev = self.graphs_dev[:self.num_test_gen]
    num_nodes_dev = [len(gg.nodes) for gg in s_graphs_dev]  # shape B X 1
    mmd_degree_dev, mmd_clustering_dev, mmd_4orbits_dev, mmd_spectral_dev = evaluate(s_graphs_dev, graphs_gen, degree_only=True)
    ...
    s_graphs_test = self.graphs_test[:self.num_test_gen]
    num_nodes_test = [len(gg.nodes) for gg in s_graphs_test]  # shape B X 1
    mmd_degree_test, mmd_clustering_test, mmd_4orbits_test, mmd_spectral_test = evaluate(s_graphs_test, graphs_gen, degree_only=True)
    ...
    fname_graphs_gen = os.path.join(self.config.save_dir, 'gran_grid_25N_{}.pickle'.format(epoch_count))
    file_gen = open(fname_graphs_gen, 'wb')
    pickle.dump(graphs_gen, file_gen)
    logger.info("Number of generated graphs are = {}".format(len(graphs_gen)))
